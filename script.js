var ac = {
   ...
   display:null,
    ...
    init : () => {
      ...
      //initialize display
      ac.display = document.getElementById('display');
    ...
    tick : () => {
      ...
      // (E3) CHECK AND SOUND ALARM
        ...
        if(ac.saveTime.length > 0){
            now = hr + min + sec;
            for(let i=0; i<ac.saveTime.length; i++){
                
                
                if(ac.saveTime[i] == now && ac.sound.paused){
                    ac.sound.play();
                }
            }
    ...
    createDiv : (list) => {
            ...
            let setTime = document.createElement("p");
            let button = document.createElement('button');
            setTime.innerHTML = list[ac.i];
            button.innerHTML = "DELETE";
            button.value=ac.i.toString();
            console.log(button.value);
            ac.display.appendChild(setTime);
            ac.display.appendChild(button);
            ac.i++;
      ...
      delete : ()=> {
      let  deleteEle = document.getElementsByTagName('button')
        deleteEle[0].onclick=  document.getElementsByTagName("p")[0].remove();
    ...
    // (F) SET ALARM
    set : () => {
      ac.alarm = ac.thr.value + ac.thm.value + ac.ths.value; //string cocatination
      ...
      if (ac.alarm != null) {
        ac.saveTime.push(ac.alarm);
        ac.alarm=null;
      }
      ac.createDiv(ac.saveTime);
    },
